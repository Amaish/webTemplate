...
import (
	"database/sql"
	"flag"
	"log" //go run cmd/web/* >>/tmp/info.log 2>>/tmp/error.log to redirect output to a file
	"net/http"
	"os"
...
func main() {
	...
	dns := flag.String("dns", "web:wordPa$2@/webTemplate?parseTime=true", "Mysql data source name")
	flag.Parse()
